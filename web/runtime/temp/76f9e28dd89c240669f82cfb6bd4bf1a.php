<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:71:"D:\wwwroot\thinkphp5_765tm6\web/application/website\view\auth\gadd.html";i:1541151551;}*/ ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset="utf-8">
		...
		<div class="layui-row layui-col-space10">
			<div class="layui-col-md9 layui-col-xs12">
				...
				<div class="layui-form-item">
					...
					<table class="layui-table">
					    ...
					    <?php if(is_array($nav) || $nav instanceof \think\Collection || $nav instanceof \think\Paginator): $i = 0; $__LIST__ = $nav;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$n): $mod = ($i % 2 );++$i;?>
					      <tr>
					        <td><input type="checkbox" name="nid" value="<?php echo $n['id']; ?>" lay-filter="Choose" <?php if(isset($v) && in_array($n['id'],$v['rules'])): ?>checked<?php endif; ?> lay-skin="primary"></td>
							...
							<input type="checkbox" name="nid" class="c<?php echo $n['id']; ?>" value="<?php echo $c['id']; ?>" <?php if(isset($v) && in_array($c['id'],$v['rules'])): ?>checked<?php endif; ?> title="<?php echo $c['title']; ?>" lay-skin="primary">
				...
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
					<?php if(isset($v['idr'])): ?>
						<input type="radio" name="isn" value="1" title="开启" lay-filter="dx" lay-skin="primary" <?php if($v['idr']): ?>checked<?php endif; ?> />
						<input type="radio" name="isn" value="0" title="关闭" lay-filter="dx" lay-skin="primary" <?php if(!$v['idr']): ?>checked<?php endif; ?>/>
					<?php else: ?>
				...
				<div class="layui-form-item" id="menu" <?php if(!(isset($v['idr']) && $v['idr'])): ?>style="display: none;"<?php endif; ?>>
					...
					<table class="layui-table">
				        ...
				        <?php if(is_array($cate) || $cate instanceof \think\Collection || $cate instanceof \think\Paginator): $i = 0; $__LIST__ = $cate;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$l): $mod = ($i % 2 );++$i;?>
				          <tr cate-id='<?php echo $l['id']; ?>' fid='<?php echo $l['pid']; ?>' >
				          	<td><input type="checkbox" name="cid" value="<?php echo $l['id']; ?>" <?php if(isset($v) && in_array($l['id'],$v['cid'])): ?>checked<?php endif; ?> lay-skin="primary"></td>
				            <td><?php echo $l['id']; ?></td>
...
layui.use(['form','layer','layedit','laydate','table'],function(){
    ...
    form.on("submit(add)",function(data){
        ...
        $.post("<?php echo url('auth/rungadd'); ?>",{
            title : $(".title").val(), 
