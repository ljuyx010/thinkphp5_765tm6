<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:71:"D:\wwwroot\thinkphp5_765tm6\web/application/website\view\link\type.html";i:1537429636;}*/ ?>
<!DOCTYPE html>
<html>
  ...
  <body>
    <div class="x-body">
      <xblock>
          <button class="layui-btn add-btn"><i class="layui-icon layui-icon-add-circle-fine"></i>增加友链分类</button>
      ...
      <table class="layui-table layui-form">
        ...
        <?php if(is_array($list) || $list instanceof \think\Collection || $list instanceof \think\Paginator): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?>
          <tr link-id='<?php echo $v['id']; ?>' fid='<?php echo $v['fid']; ?>' >
            <td><?php echo $v['id']; ?></td>
    ...
    function addmenu(id,e){
        if(e==1){
          var url="<?php echo url('link/typedit','',''); ?>/id/"+id;
          var p="修改";
        }else{
          var url="<?php echo url('link/typeadd','',''); ?>";
          var p="添加";
        }
  ...
  $("input").blur(function(){
    var v=$(this).val();
    var id=$(this).attr("data-id");
    $.post("<?php echo url('link/runtype','',''); ?>",{'id':id,'order':v},function(result){
      if(result==0){
        layer.msg('排序失败!');
      }
    });
  ...
  function menu_del(obj,id){
      layer.confirm('确认要删除吗？',function(index){            
          //发异步删除数据
        $.post("<?php echo url('link/typedel','',''); ?>",{id:id},function(result){
          if(result){
